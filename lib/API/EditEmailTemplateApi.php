...
class EditEmailTemplateApi extends ApiValidatorAbstract implements ApiInterface
{
    use ResponseTraits;
    ...
    function run(): void
    {
        $setting = SettingsModel::firstOrCreate(
            [
                'key' => 'email_template',
            ]
        );

        $setting->val = $_REQUEST['template_id'];
        $setting->saveOrFail();
