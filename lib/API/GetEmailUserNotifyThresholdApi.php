...
class GetEmailUserNotifyThresholdApi extends ApiValidatorAbstract implements ApiInterface
{
    use ResponseTraits;
    ...
    function run(): void
    {
        ...
        if (empty($notify) || empty($threshold =$notify->threshold)) {
            $threshold = 0;
        }
        $this->responseData('success', ['threshold' => $threshold]);
    }
