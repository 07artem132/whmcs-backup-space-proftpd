...
class GetEmailTemplateApi extends ApiValidatorAbstract implements ApiInterface
{
    use ResponseTraits;
    ...
    function run(): void
    {
        ...
        $result = SettingsModel::where('key', '=', 'email_template')->first();

        if (!empty($result))
            $status = (int)$result->val;
        else
            $status = 0;

        $this->responseData('success', ['email_template' => $status]);
    }
